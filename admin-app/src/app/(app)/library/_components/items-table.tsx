          ...
          <TableBody>
            ...
            {data?.items.map((it: Item) => (
              <TableRow key={it.id} className="hover:bg-accent/50">
                <TableCell><Link href={`/library/${it.id}`} className="font-medium hover:underline">{it.title}</Link></TableCell>
            ...
            {data && data.items.length === 0 && (
              <TableRow><TableCell colSpan={4} className="text-muted-foreground">No items.</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </div>
