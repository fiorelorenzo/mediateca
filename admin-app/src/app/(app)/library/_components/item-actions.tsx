...
export function ItemActions({ item }: { item: Item }) {
  ...
  return (
    <div className="flex flex-wrap gap-2">
      <Button disabled={pending} variant="default" onClick={() => start(async () => { await api.searchNow(item.id); refresh(); })}>
        Search now
      </Button>
      <Button disabled={pending} variant="outline" onClick={() => start(async () => { await api.acceptAsIs(item.id); refresh(); })}>
        Accept as-is
      </Button>
...
function OverrideDialog({ item, onSaved }: { item: Item; onSaved: () => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild><Button variant="secondary">Override policy</Button></DialogTrigger>
      <DialogContent>
        <DialogHeader><DialogTitle>Per-item language policy</DialogTitle></DialogHeader>
        <div className="space-y-2">
          <Label htmlFor="langs">Required audio languages (comma-separated, supports `@original`)</Label>
