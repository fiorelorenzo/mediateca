    ...
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      <Card>
        ...
        <CardContent>
          ...
          <div className="text-sm text-muted-foreground">
            {load_avg["5m"].toFixed(2)} (5m) / {load_avg["15m"].toFixed(2)} (15m) · {cpu_count}{" "}
            cores
          </div>
        </CardContent>
      </Card>
      <Card>
        ...
        <CardContent>
          ...
          <div className="text-sm text-muted-foreground">
            {formatBytes(used)} of {formatBytes(mem.total_kb)} used
          </div>
        </CardContent>
      </Card>
      <Card>
        <CardHeader>
          <CardTitle>Disk (data)</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="text-3xl font-bold">{diskPct}%</div>
          <div className="text-sm text-muted-foreground">
            {(disk_data.free / 1e9).toFixed(1)} GB free of {(disk_data.total / 1e9).toFixed(1)} GB
          </div>
        </CardContent>
      </Card>
    </div>
